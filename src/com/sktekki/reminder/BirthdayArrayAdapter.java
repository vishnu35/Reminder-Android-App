...
public class BirthdayArrayAdapter extends ArrayAdapter<BirthdayBean> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			view = inflater.inflate(com.sktekki.reminder.R.layout.view_list_item, parent, false);
		} else {
			view = convertView;
		}

		// Set RowId in the UI
		TextView rowId_text = (TextView) view.findViewById(com.sktekki.reminder.R.id.rowId);
		if (rowId_text != null) {
			rowId_text.setText(String.valueOf(bean.getRowId()));
		}

		// Set Photo in the UI
		ImageView mPhotoView = (ImageView) view.findViewById(com.sktekki.reminder.R.id.photo_id);
		Bitmap bitmap = loadContactPhoto(this.context.getContentResolver(),
				bean.getContactId());
		if (bitmap != null) {
			mPhotoView.setImageBitmap(bitmap);
		} else {
			mPhotoView.setImageResource(com.sktekki.reminder.R.drawable.android);
		}

		// Set Name in the UI
		TextView name_text = (TextView) view.findViewById(com.sktekki.reminder.R.id.txtName);
		if (name_text != null) {
			name_text.setText(bean.getName());
		}

		// Set birthdate in the UI
		TextView birthday = (TextView) view.findViewById(com.sktekki.reminder.R.id.txtBirthday);
		birthday.setText(bean.getBirthday());

		// Set Remaining Days in the UI
		TextView birthDayText = (TextView) view
				.findViewById(com.sktekki.reminder.R.id.txtBirthdayText);
		...
		CheckBox chkReminder = (CheckBox) view.findViewById(com.sktekki.reminder.R.id.chkReminder);
		...
		// Set the sync image
		ImageView calImage = (ImageView) view.findViewById(com.sktekki.reminder.R.id.syncIcon);
		if (bean.isReminder()) {
